...
async def command_sfw_get_url(message: Message, get_url: GetUrl, database: RedisDB):
    ...
    try:
        ...
        text = get_text(lang_code=message.from_user.language_code, text_name="many_duplicates")
    ...
    if text is None:
        ...
        try:
            ...
            asyncio.get_event_loop().call_later(
                ...
                command_sfw_get_url(message, get_url, database)
            )
    else:
...
async def command_nsfw_get_url(message: Message):
    ...
    markup = create_inline_keyboard_markup(
        inline_keyboard=[
...
async def command_send_nsfw_url(callback: CallbackQuery, get_url: GetUrl, database: RedisDB):
    ...
    try:
        ...
        text = get_text(lang_code=callback.from_user.language_code, text_name="many_duplicates")
    except asyncio.TimeoutError:
    ...
    if text is None:
        ...
        try:
            ...
            asyncio.get_event_loop().call_later(
                0.2, asyncio.create_task,
                command_send_nsfw_url(callback, get_url, database)
            )
    else:
